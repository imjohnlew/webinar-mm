...
const CalculatorVisualFunnel = ({ metrics, isDynamic, isExpanded = false, showProfitSplit = false, showAdSpend = false, comparisons = {}, handlers, directMode = false, directLeadsValue, setDirectLeads }) => {
    ...
    // Calculate raw unrestricted widths based on conversion rate
    ...
    // Apply strict funnel constraints (lower layers MUST be visibly narrower than the layer above them)
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left text-sm text-slate-300">
                    <thead className="bg-slate-800/80 text-xs uppercase font-bold text-slate-400 border-b border-slate-700">
                        <tr>
                            <th className="px-4 py-3 rounded-tl-lg">Webinars</th>
                    ...
                    <tbody className="divide-y divide-slate-700/50">
                        {multipliers.map((m) => (
                            <tr key={m} className={`hover:bg-slate-800/30 transition-colors ${m === 1 ? 'bg-slate-800/10' : ''}`}>
                                <td className="px-4 py-3 font-bold text-white whitespace-nowrap">{m} <span className="text-slate-500 font-normal text-xs ml-1">session{m > 1 ? 's' : ''}</span></td>
...
const WebinarCalculator = () => {
    ...
    const calculateMetrics = (t, s, o, a, c, p, title, directLeadsOverride = null) => {
        ...
        const profit = revenue - s; // Revenue - Ad Spend
    ...
    const handlePresetChange = (e) => {
        ...
        if (preset) {
            setCurOptIn(preset.optIn);
            setCurApptRate(preset.apptRate);
            setCurCloseRate(preset.closeRate);
        }
    };
        ...
        <div className="w-full flex justify-center py-8">
            <div className="w-full max-w-[1400px] flex flex-col gap-12">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                    <div>
                        <h2 className="text-3xl font-black text-white tracking-tight">Webinar Calculator</h2>
                ...
                <div className="flex flex-col lg:flex-row gap-8 w-full">
                    ...
                    <div className={`flex lg:flex-row flex-col gap-12 flex-1 items-start justify-center bg-slate-900 border border-slate-800 p-10 rounded-[2rem] shadow-xl transition-all duration-500`}>
                        ...
                        {showProjected && (
                            <>
                                <div className="hidden lg:flex items-center justify-center py-24 text-slate-700 text-6xl font-light animate-pulse">
                                    →
                                </div>
