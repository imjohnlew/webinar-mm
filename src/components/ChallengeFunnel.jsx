...
const ChallengeRegMockup = () => (
    <div className="bg-white text-[9px] text-slate-700">
        <div className="bg-gradient-to-br from-violet-700 to-purple-800 px-3 py-4 text-center">
            <div className="text-violet-200 text-[7px] font-bold uppercase tracking-widest mb-1">FREE 5-DAY CHALLENGE</div>
            <div className="text-white font-black text-[12px] leading-tight mb-1">
                Join The FREE<br/>[Result] Challenge
            </div>
            <div className="text-violet-200 text-[8px]">Starting [Date] · 100% Free · [X] Spots Left</div>
        </div>
        <div className="p-3 space-y-2">
            <div className="text-center text-[8px] text-slate-600">In just 5 days, you'll go from [current state] to [desired outcome] — even if you're starting from zero.</div>
            <div className="grid grid-cols-5 gap-1">
                {['Day 1', 'Day 2', 'Day 3', 'Day 4', 'Day 5'].map((d, i) => (
                    <div key={d} className="bg-violet-50 border border-violet-200 rounded p-1 text-center">
                        <div className="text-[6px] font-black text-violet-600">{d}</div>
            ...
            <div className="space-y-1.5">
                ...
                <div className="w-full bg-violet-600 text-white font-black text-[10px] py-2 rounded text-center">
...
const WelcomeMockup = () => (
    <div className="bg-white text-[9px] text-slate-700">
        <div className="bg-gradient-to-r from-violet-600 to-purple-600 px-3 py-3 text-center">
            ...
            <div className="text-white font-black text-[10px]">You're In! Welcome to the Challenge!</div>
            <div className="text-violet-100 text-[7px]">Day 1 starts on [DATE]</div>
        </div>
        <div className="p-3 space-y-2.5">
            <div className="text-[8px] text-slate-600 text-center">We're so excited to have you. Here's everything you need to get ready.</div>
            {/* Step 1: Join community */}
            <div className="bg-violet-50 border border-violet-200 rounded-lg p-2 space-y-1">
                <div className="font-bold text-violet-700 text-[8px]">Step 1: Join Our Private Community</div>
                <div className="text-[7px] text-violet-600">This is where we do the daily lives — don't miss it!</div>
                <div className="w-full bg-violet-600 text-white font-bold text-[8px] py-1.5 rounded text-center">
                    👥 Join [Group Name] on Facebook →
                </div>
            </div>
            {/* Step 2: Intro video */}
            <div className="space-y-1">
                <div className="font-bold text-slate-700 text-[8px]">Step 2: Watch Your Day 0 Prep Video (8 min)</div>
            ...
            <div className="border border-slate-200 rounded-lg p-2 space-y-1">
                <div className="font-bold text-slate-700 text-[8px]">What to Expect:</div>
                {['Daily live training at [TIME]', 'Short action item each day (~15 min)', 'Community support + accountability'].map(s => (
                    <div key={s} className="flex items-center gap-1 text-[8px] text-slate-600">
...
const DailyContentMockup = () => (
    <div className="bg-white text-[9px] text-slate-700">
        <div className="bg-violet-600 px-3 py-2 flex items-center justify-between">
            <div className="text-white font-black text-[9px]">🏆 Day 3 of 5</div>
            <div className="flex gap-1">
                {[1,2,3,4,5].map(d => (
                    <div key={d} className={`w-4 h-4 rounded-full flex items-center justify-center text-[6px] font-black ${d <= 3 ? 'bg-white text-violet-700' : 'bg-violet-500 text-violet-200'}`}>{d <= 2 ? '✓' : d === 3 ? '●' : d}</div>
                ))}
            </div>
        </div>
        <div className="p-3 space-y-2">
            <div className="font-black text-slate-900 text-[10px]">Day 3: [The Turning Point]</div>
            <div className="text-[7px] text-slate-500">Today's lesson: How to [specific skill] so that [outcome]</div>
            {/* Video */}
            <div className="w-full aspect-video bg-slate-800 rounded flex items-center justify-center relative">
                ...
                <div className="absolute bottom-1.5 left-0 right-0 h-1 bg-slate-600 mx-2 rounded-full">
                    <div className="w-1/3 h-full bg-violet-400 rounded-full" />
                </div>
                <div className="absolute top-1.5 right-1.5 bg-black/60 text-white text-[6px] px-1 py-0.5 rounded">14:32</div>
            </div>
            {/* Today's action */}
            <div className="bg-yellow-50 border border-yellow-300 rounded-lg p-2 space-y-1">
                <div className="font-bold text-yellow-700 text-[8px]">⚡ Today's Action Item</div>
                <div className="text-[7px] text-yellow-600">[Specific task that creates momentum and results]</div>
                <div className="border border-yellow-300 rounded px-2 py-1 bg-white">
                    <Bar w="w-full" h="h-1.5" color="bg-yellow-200" />
                </div>
                <div className="w-full bg-yellow-400 text-yellow-900 font-black text-[8px] py-1 rounded text-center">Mark as Complete ✓</div>
            </div>
            {/* Community */}
            <div className="flex items-center justify-between text-[7px] text-slate-500 bg-slate-50 rounded p-1.5">
...
const PitchDayMockup = () => (
    <div className="bg-white text-[9px] text-slate-700">
        <div className="bg-gradient-to-r from-orange-500 to-red-500 px-3 py-3 text-center">
            ...
            <div className="text-orange-100 text-[7px] mt-0.5">Join us LIVE today at [TIME] — something special is waiting</div>
        </div>
        <div className="p-3 space-y-2">
            {/* Transformation recap */}
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 space-y-1">
                <div className="font-bold text-slate-700 text-[8px]">Look How Far You've Come:</div>
                {['✅ Day 1: [Foundational win]', '✅ Day 2: [Progress milestone]', '✅ Day 3: [Turning point]', '✅ Day 4: [Momentum builder]', '🏁 Day 5: [Final transformation]'].map(s => (
                    <div key={s} className="text-[7px] text-slate-600">{s}</div>
                ))}
            </div>
            {/* Live countdown */}
            <div className="border-2 border-orange-400 bg-orange-50 rounded-lg p-2 text-center space-y-1">
                <div className="font-black text-orange-700 text-[9px]">LIVE STARTS IN:</div>
                ...
                <div className="w-full bg-orange-500 text-white font-black text-[9px] py-1.5 rounded">Join The Live →</div>
            </div>
            {/* Teaser of offer */}
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-2 text-center space-y-1">
                <div className="font-bold text-blue-700 text-[8px]">🎁 Challenge Graduates Exclusive:</div>
                <div className="text-[7px] text-blue-600">We're revealing something special for those who completed all 5 days. This offer is ONLY available today.</div>
                <div className="text-[7px] text-blue-500 font-bold">Show up to find out →</div>
            </div>
        </div>
    </div>
);
...
const ChallengeOrderMockup = () => (
    <div className="bg-white text-[9px] text-slate-700">
        <div className="bg-orange-500 px-3 py-2 text-center">
            <div className="text-white font-black text-[9px]">🏆 Challenge Graduate Special Offer</div>
            <div className="text-orange-100 text-[7px]">Available for challenge completers only — expires in 24 hours</div>
        </div>
        <div className="p-3 space-y-2">
            <div className="font-black text-center text-slate-900 text-[10px] leading-tight">
                "Continue Your Momentum With [Program Name]"
            </div>
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 space-y-1">
                <div className="font-bold text-slate-700 text-[8px]">What Comes Next:</div>
            ...
            <div className="text-center space-y-0.5">
                <div className="text-[8px] text-orange-600 font-bold">Challenge Graduate Price:</div>
                <div className="text-[8px] text-slate-400 line-through">$997</div>
                <div className="text-2xl font-black text-slate-900">$497</div>
                <div className="text-[7px] text-slate-500">or 2 payments of $297</div>
            </div>
...
const ChallengeTYMockup = () => (
    <div className="bg-white text-[9px] text-slate-700">
        <div className="bg-gradient-to-r from-violet-600 to-purple-600 px-3 py-3 text-center">
            ...
            <div className="text-violet-100 text-[7px]">Your transformation continues — deeper, faster</div>
        </div>
        <div className="p-3 space-y-2">
            <div className="text-[8px] text-slate-600 text-center">You've taken the challenge — now you're going all in. Access is live.</div>
            <div className="w-full bg-violet-600 text-white font-black text-[9px] py-2 rounded text-center">Start [Program Name] →</div>
            <div className="border border-slate-200 rounded-lg p-2 space-y-1.5">
                <div className="font-bold text-slate-700 text-[8px]">Your First 24 Hours:</div>
                {['Watch the welcome video and orientation', 'Introduce yourself in the VIP community', 'Complete the Day 1 deep-dive module'].map((s, i) => (
                    <div key={s} className="flex items-center gap-1.5 text-[8px] text-slate-600">
                        <div className="w-3.5 h-3.5 rounded-full bg-violet-100 text-violet-600 font-black flex items-center justify-center text-[7px] shrink-0">{i + 1}</div>
                        {s}
                    </div>
                ))}
            </div>
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 text-center">
                <div className="text-[7px] text-slate-500 italic">"The challenge was amazing but [Program Name] is where the real transformation happened."</div>
                <div className="text-[7px] font-bold text-slate-600 mt-0.5">— [Graduate Name]</div>
            </div>
        </div>
    </div>
);
